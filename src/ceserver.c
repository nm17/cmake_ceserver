...
#define COMPRESS_BLOCKSIZE (64*1024)
                    ...
                    int maxBlocks = (int)(1 + (c.size / COMPRESS_BLOCKSIZE));

                    unsigned char **compressedBlocks = malloc(maxBlocks *
                                                              sizeof(unsigned char *)); //send in blocks of 64kb and reallocate the pointerblock if there's not enough space
                    int currentBlock = 0;
                    ...
                    for (i = 0; i <= currentBlock; i++) {
                        if (i != currentBlock)
                            ...
                            sendall(currentsocket, compressedBlocks[i], COMPRESS_BLOCKSIZE - strm.avail_out,
                                    0); //last one, flush
...
#pragma clang diagnostic ignored "-Wint-to-void-pointer-cast"
            pthread_create(&pth, NULL, (void *) CESERVERTEST, (void *) TEST_PID);
