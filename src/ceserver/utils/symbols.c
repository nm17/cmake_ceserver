...
*/
{
    ...
    *output = malloc(maxoutputsize); //allocate 256KB. This "should" be enough, but reallocate if more is needed
    ...
    strm.avail_out = maxoutputsize - 3 *
                                     sizeof(uint32_t); //first if it's an exe, followed by the compressed size, followed by the decompressed size
...
*/
{
    ...
    *output = malloc(maxoutputsize); //allocate 256KB. This "should" be enough, but reallocate if more is needed
    ...
    strm.avail_out =
            maxoutputsize - 3 * sizeof(uint32_t); //the first 8 bytes will contain the compressed and uncompressed size
    strm.next_out = (unsigned char *) &(*output)[sizeof(uint32_t) * 3];
